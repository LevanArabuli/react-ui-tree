...
class UITreeNode extends Component {
  ...
  renderChildren = (skipPadding) => {
    const { index, tree, dragging } = this.props;

    if (index.children && index.children.length) {
      const childrenStyles = {
        paddingLeft: skipPadding ? '' : this.props.paddingLeft
      };
  ...
  render() {
    ...
    const shouldRenderCollapse = this.props.index.children && this.props.index.children.length > 0;
    const isCollapsed = this.props.index.node.collapsed;

    if (index.id === 1 && this.props.skipFirstNode) {
    ...
    return (
      ...
      >
        ...
        >
          {tree.renderNode(node, shouldRenderCollapse, isCollapsed, this.handleCollapse)}
        </div>
